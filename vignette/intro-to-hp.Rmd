...
The package contains for colour scales: One for each house of Hogwarts School.
...
plots: `scale_color_hp()` and `scale_fill_hp()`.  As with `hp()`, you can use the other scales with the `house` argument
...
p + geom_point(size=4, aes(colour = factor(carb))) +
    scale_color_hp(discrete=TRUE, house = "ravenclaw") +
    theme_bw()
```
